...
<div class="carousel-container">
  <button class="prev-button" @click="${() => goToSlide(state.currentSlide - 1)}"></button>
  <button class="next-button" @click="${() => goToSlide(state.currentSlide + 1)}"></button>
  <div class="carousel-inner">
    ${() => state.slides.map((slide, index) =>
      t`
        <figure
          class="slide"
          style="${() => `transform: translateX(${index - state.currentSlide}00%);opacity: ${index === state.currentSlide ? '1' : '0.5'};`}"
        >
